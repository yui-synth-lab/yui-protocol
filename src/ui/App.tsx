...
import DynamicThreadView from './DynamicThreadView';
                ...
                {showCreateForm && (
                  <form onSubmit={handleCreateSession} className="mb-4 space-y-3">
                    ...
                    <div>
                      ...
                      <select 
                        value={version} 
                        onChange={e => setVersion(e.target.value as '1.0' | '2.0')}
                        className="w-full px-3 py-2 text-sm border border-gray-700 bg-gray-800 text-gray-100 focus:ring-2 focus:ring-blue-800 focus:border-transparent rounded"
                      >
                        <option value="1.0">v1.0 - Fixed 5-Stage Process</option>
                        <option value="2.0">v2.0 - Dynamic Consensus-Based Dialogue</option>
                      </select>
                    </div>
...
export function AppRoutes() {
  ...
  const [protocolVersion, setProtocolVersion] = useState<'1.0' | '2.0'>('1.0');
  ...
  const createNewSession = async (title: string, selectedAgentIds: string[], language: string, version: '1.0' | '2.0') => {
    try {
      console.log('App: Creating session with:', { title, selectedAgentIds, language, version });
      ...
      const response = await fetch('/api/sessions', {
        ...
        body: JSON.stringify({ title, agentIds: selectedAgentIds, language, version }),
      });
          ...
          <header className="flex-shrink-0 mb-4">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-xs text-gray-500">
                {protocolVersion === '1.0' ? (
                  <span className="text-blue-400">Fixed 5-Stage Process</span>
                ) : (
                  <span className="text-purple-400">Dynamic Consensus-Based Dialogue</span>
                )}
              </div>
            </div>
            ...
            {protocolVersion === '1.0' ? (
              ...
              <div className="bg-purple-900 border border-purple-700">
                <button
                  onClick={() => setShowProcessInfo(!showProcessInfo)}
                  className="w-full p-2 text-left flex items-center justify-between hover:bg-purple-800 transition-colors"
                >
                  <h3 className="text-sm font-medium text-purple-200">Dynamic Consensus-Based Dialogue</h3>
                  <svg
                    className={`w-4 h-4 text-purple-300 transition-transform ${showProcessInfo ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {showProcessInfo && (
                  <div className="px-2 pb-2">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                      <div className="bg-purple-800 p-2 text-purple-100">
                        <strong>🧠 Memory Hierarchy</strong><br/>
                        <span className="text-xs">Recent (full) → Summary (compressed) → Essence (keywords)</span>
                      </div>
                      <div className="bg-indigo-800 p-2 text-indigo-100">
                        <strong>🤝 Consensus Tracking</strong><br/>
                        <span className="text-xs">Satisfaction levels, additional points, readiness to conclude</span>
                      </div>
                      <div className="bg-pink-800 p-2 text-pink-100">
                        <strong>🎭 Facilitator Actions</strong><br/>
                        <span className="text-xs">Deep dive, clarification, perspective shift, summarize</span>
                      </div>
                    </div>
                    <div className="mt-2 p-2 bg-purple-800 text-purple-100 text-xs">
                      <strong>Process:</strong> Agents continue dialogue until reaching natural consensus or satisfaction threshold (max 20 rounds)
                    </div>
                  </div>
                )}
              </div>
